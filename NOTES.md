...
- Node 22: `npm test` covers actual editing topology and drag projection/inversion, multiple camera positions/distances, end-on fallback, signed depth and supporting-layer bounds, sub-threshold no-op, preview topology stability and source immutability.
...
- Browser cases: foreground face selection, click-only selection, multiple previews with zero undo entries until release, one-step commit/undo, Escape, pointer cancellation, return-to-zero, bevel height/width dragging, shortening existing extrusions without adding faces, keyboard Enter, background/right orbit, cached-tool deactivate rollback, and 390px touch dragging/no horizontal overflow.
